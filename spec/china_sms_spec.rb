...
describe "ChinaSMS" do
  ...
  context 'with service' do
    before {
      ChinaSMS.use service2, username: username, password: password ,scope: :global
    ...
    describe "#use" do
      ...
      it "username is saberma" do
        expect(chinasms.accounts["tui3_domestic".to_sym][:username]).to eq "saberma"
      end
    end
    ...
    describe "to nexmo" do
      before {
        allow(ChinaSMS::Service::Nexmo).to receive(:to).with(phone,content,{:username => username,:password => password,:scope => :global}).and_return(success: true, code: 0)
      }
      subject { ChinaSMS.to(phone,content,:scope => :global) }
      its([:success]) { should eql true }
      its([:code]) { should eql 0 }
    end
  end
